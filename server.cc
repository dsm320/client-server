 ...
 * Server.cc
 * 
 * Server half of client/server pair that demonstrates how a server can use
 ...
 * Store them in the arg_t struct and populate via the get_args()
 * function.
 */
struct arg_t {
 ...
 * program.  This routine makes it easier to see the logic in the program while
...
/**
 * Create a server socket that can listen for new incoming requests
 *
 * @param port The port on which the program should listen for new connections
 */
int create_server_socket(std::size_t port) {
  // A socket is just a kind of file descriptor. Connections to use
  // IPV4 and TCP:
...
/**
 * When a client sends a message, use this to read from the client socket.
 *
...
bool handle_client_input(int sd) {
  ...
  // Otherwise, print whatever data is received
  else {
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    // process them.
    //
    // NB: FD_SETSIZE is a constant, defined as 1024 in Linux.
    for (int i = 0; i < FD_SETSIZE; ++i) {
      if (FD_ISSET(i, &read_fd_set)) {
        // if this socket is the server socket, it means there is a new incoming
        // connection that needs to add to the set.
        if (i == serverSd) {
