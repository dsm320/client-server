 ...
 * Client.cc
 * 
 * Client half of client/server pair that demonstrates how a server can use
 * select() so that single-threaded server can manage multiple clients.
 * 
 * After starting server, run multiple copies of this program from a loop:
 *   bash:> for i in 'seq 12'; do ./obj64/client.exe -p [server port] -s [server name] -w $i & done
 */
...
/**
 * Display a help message to explain command-line parameters
 *
...
/**
 * Print an error message that combines a prefix with the
 * standard unix error message that accompanies errno, then exit the
 * program
 *
...
/**
 * Connect to a server so that for bidirectional communication on the
 ...
 * @param port     The server's port that should be used
 */
int connect_to_server(std::string hostname, std::size_t port) {
  // figure out the IP address that is needed to use and put it in a sockaddr_in
...
int main(int argc, char **argv) {
  ...
  // Use the program's process id to uniquely identify it, so that the
